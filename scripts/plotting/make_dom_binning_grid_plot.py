...
def scan_for_highest_distance(x_one_layer,y_one_layer,x_bin_edges,y_bin_edges, x_offsets, y_offsets, multiply=[1], efficient=False):
...
def maximize_distance():
    ...
    distances = scan_for_highest_distance(x_red,y_red,x_bin_edges,y_bin_edges,x_offsets,y_offsets, multiply, efficient)
    ...
    print("Best x-offset:",best_x, "Best y-offset", best_y, "Best factor: ", best_factor, "Distance at this point:", best_dist)
...
def show_distance_of_bins(x_one_layer, y_one_layer, x_bin_edges, y_bin_edges, offsets):
    for offset in offsets:
        ...
        distances = np.sort(np.min([x_unit_cell, box_length_x-x_unit_cell, y_unit_cell, box_length_y-y_unit_cell], axis=0))
...
def get_distance_to_edges_minimizer( x, args ):
    ...
    distance = -1 * np.min([np.min(x_unit_cell), np.min(box_length_x-x_unit_cell), np.min(y_unit_cell), np.min(box_length_y-y_unit_cell)] )
