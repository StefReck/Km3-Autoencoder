...
def parse_input():
    parser = argparse.ArgumentParser(
        description='Make some plots of original event vs autoencoder reco and show them.')
    parser.add_argument('model', metavar="m", type=str, 
                        help='The model that does the predictions.')
    parser.add_argument('dataset_tag', metavar="d", type=str, 
                        help='Dataset to use.')
    
    parser.add_argument('how_many', metavar="h", type=int, nargs="?", 
                        default=4, help='How many plots of events will be generated.')
    parser.add_argument('energy_threshold', metavar="e", type=float, nargs="?", 
                        default=0, help='Minimum energy of events for them to be considered')
